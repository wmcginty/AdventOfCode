...
struct Grid: CustomStringConvertible {
    ...
    mutating func performRound() -> Bool {
        ...
        for coord in elfCoordinates {
            ...
            case .east: proposedMoves[coord] = coord.moved(in: .east, amount: 1, rowLimit: nil)
            case .west: proposedMoves[coord] = coord.moved(in: .west, amount: 1, rowLimit: nil)
            case .south: proposedMoves[coord] = coord.moved(in: .south, amount: 1, rowLimit: nil)
            case .north: proposedMoves[coord] = coord.moved(in: .north, amount: 1, rowLimit: nil)
            }
        }
    ...
    func areNeighborsAllEmpty(at coordinates: Set<Coordinate>) -> Bool {
...
try measure(part: .one) {
    try groundTilesInSmallestRectangleContainingAllElvesAfter(rounds: 10, from: .input)
}

try measure(part: .two) {
    try numberOfRoundsBeforeNoElfNeedsToMove(from: .input)
}
