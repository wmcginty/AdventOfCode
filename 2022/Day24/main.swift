...
class Storm {
    ...
    func moveBlizzards(_ blizzards: Set<Blizzard>) -> Set<Blizzard> {
        ...
        for var blizzard in blizzards {
            ...
            let moved = blizzard.coordinate.moved(in: blizzard.direction, amount: 1, rowLimit: nil)
...
class Solver {
    ...
    init(valley: Valley, storm: Storm) {
        self.valley = valley
        self.storm = storm
        self.possibleBlizzardStates = valley.floorWidth.leastCommonMultiple(with: valley.floorHeight)
    ...
    func minimumStepsAcrossValley(times: Int = 1) -> Int {
        ...
        while let state = deque.popFirst() {
            ...
            for direction in availableDirectionsForPlayer(at: state.player) {
                let moved = state.player.moved(in: direction, amount: 1, rowLimit: nil)
