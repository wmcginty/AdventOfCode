...
struct Piece: CaseIterable {
    ...
    func moved(to direction: Coordinate.Direction, amount: Int = 1) -> Piece {
        Piece(coordinates: coordinates.map { $0.moved(in: direction, amount: amount, rowLimit: nil, columnLimit: nil) }, height: height)
    }
...
class Chamber {
    ...
    func skyline() -> [Coordinate] {
        ...
        return points.map { $0.moved(in: .south, amount: -maxY, rowLimit: nil, columnLimit: nil) }
    }
}
...
measure(part: .one) {
    heightOfTetris(after: 2022, from: .input)
}

measure(part: .two) {
    heightOfTetris(afterMany: 1_000_000_000_000, from: .input)
}
